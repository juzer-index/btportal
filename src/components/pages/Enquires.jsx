...
const COLUMN = [
  ...
  //     Cell: (props) => <div style={{textAlign:"left"}} className="text-left float-left">

  //     <Link
  //       //   to={`/admin_customers_by_id/${props.cell.row.original._id}`}
  ...
  headersColumn("Message", "message"),
  headersColumn("Trip", "trip"),
  {
  ...
  },
  {
    ...
    Cell: (props) => (
      <>
        <div>
          <Link
            to={`/admin_enquiry_details/${props.value}`}
...
const Enquires = (props) => {
  ...
  const getData = useCallback(async () => {
    setLoading(true);
    try {
      const apiData = await getEnquires();
      if (apiData.error) {
        ...
        if (type === "my") {
          ...
          const filteredData = apiData.data.filter(
            (item) => item.contactPerson && item.contactPerson._id === user_id
          );
  ...
  return (
    <div className={styles.DataTable}>
      <PageHeader titles="Enquires" active="Customers" items={["Tables"]} />
      <ToastContainer />
