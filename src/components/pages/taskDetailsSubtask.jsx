...
const TaskDetails = () => {
  ...
  const getData = useCallback(async () => {
    ...
    try {
      ...
      if (apiData.error) {
        ...
        const filterData = apiData?.data?.todo?.find((data) => {
          return data?._id === taskid;
        });
        console.log("getData getTaskById", taskid, filterData);
        setMainData(filterData);
        setMainData2(apiData?.data);
      }
  ...
  const addData = async (body) => {
    ...
    try {
      const apiData = await addFilesInSubTask(body);
      if (apiData.error) {
        setError(apiData.error);
      } else {
        getData();
      ...
      console.log("err2123", err);
      if (err.response) {
        setError(err.response.data.message);
        setErrorToast(err.response.data.message);
      } else if (err.name === "AxiosError") {
        setError(
          "Uploaded File is to large , please upload files less than 5MB"
        );
        setErrorToast(
          "Uploaded File is to large , please upload files less than 5MB"
        );
  ...
  return (
    <div className={styles.DataTable}>
      <PageHeader
        titles={`Task Name : ${mainData2?.name} & Sub Task Task Name : ${mainData?.name}`}
        active="Retailers"
        items={["Tables"]}
      />
