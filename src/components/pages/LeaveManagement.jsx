...
const LeaveManagement = (props) => {
  ...
  const COLUMN = useMemo(
    () => [
      ...
      },
      {
        ...
        Cell: (props) => (
          <div style={{ textAlign: "center" }}>
            <span>{props.value?.firstName}</span>
          </div>
        ),
      },
      headersColumn("Type", "type"),
      headersColumn("Description", "description"),
      {
        ...
        Cell: (props) => (
          ...
          //   <span>
          //     {props.value ? (
          //       <a href={props.value} className="btn btn-info" download={true}>
          ...
          <div style={{ textAlign: "center" }}>
            {props.value && (
              <a href={props.value} className="btn btn-info" download>
                <i className="fa fa-download"></i>
              </a>
            )}
          </div>
      ...
      headersColumn("Start Date", "startDate"),
      headersColumn("End Date", "endDate"),
      headersColumn("Total Days", "totalDays"),
      ...
      },
      {
        ...
        Cell: (props) => (
          <>
            <div>
              {/* <Link
                to={`/admin_section_details/${props.value}`}
  ...
  const getData = useCallback(async () => {
    ...
    try {
      ...
      if (apiData.error) {
        ...
        const isAdmin = user_type === "Admin" || user_type === "Executive";
        if (isAdmin) {
          setMainData(apiData.data);
        } else {
          const data = apiData?.data?.filter((data) => {
            return data?.userId?._id === localStorage.getItem("user_id");
          });
          setMainData(data);
        }
      }
  ...
  const addData = async (body) => {
    ...
    try {
      const apiData = await createLeave(body);
      if (apiData.error) {
        setError(apiData.error);
      } else {
        getData();
  ...
  const updateData = async (body) => {
    ...
    try {
      const apiData = await updateLeave(body);
      if (apiData.error) {
        setError(apiData.error);
      } else {
        getData();
  ...
  const { onDownload } = useDownloadExcel({
    ...
    filename: "Leave Application Table",
    sheet: "Leave",
  });
  ...
  return (
    <div className={styles.DataTable}>
                        ...
                        <div className="mb-3">
                          ...
                          <select name="type" className="form-control" id="">
                            <option value="Annual leave">
                              Annual leave (12 in year){" "}
                            </option>
                            <option value="Causal leave">
                              Casual leave (12 in year)
                            </option>
                            <option value="Sick leave">
                              Sick leave ( 5 in year){" "}
                            </option>
                      ...
                      <>
                        <div className="col-lg-2 ml-2">
                          <Link
                            ...
                            className="me-3 mt-2 btn btn-success"
                          >
                            Pending Leave
                          </Link>
                        </div>
                        <div className="col-lg-2">
                          <Link
                            className="me-3 mt-2 btn btn-warning"
                            to={"/admin/approvedLeave"}
                          >
                            Approved Leave
                          </Link>
                        </div>
      ...
      <Modal size="l" show={largeShow} onHide={largemodalClose}>
        <ModalHeader>
          <ModalTitle>Update Details</ModalTitle>
                ...
                <div className="col-lg-12">
                  <div className="mb-3">
                    ...
                    >
                      ...
                      <option value="Public Holiday">12 Public Holiday</option>
                    </select>
                    <input
                      type="hidden"
                ...
                <div className="col-lg-12">
                  <div className="mb-3">
                    ...
                    <input
                      ...
                      name="totalDays"
                      placeholder="Enter Total Days"
                      defaultValue={modalData && modalData?.totalDays}
                    />
                    <input
                      type="hidden"
                      value={modalData && modalData?._id}
