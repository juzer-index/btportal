...
const Templates = (props) => {
  ...
  const COLUMN = useMemo(
    () => [
      ...
      headersColumn("Template Name", "name"),
      {
        ...
        Cell: (props) => (
          <>
            <div>
              <Link
                to={`/templateDetails/${props.value}`}
  ...
  const addData = async (body) => {
    ...
    try {
      const apiData = await createTemplate(body);
      if (apiData.error) {
        setError(apiData.error);
      } else {
        getData();
  ...
  const updateData = async (body) => {
    ...
    try {
      const apiData = await updateTemplate(body);
      if (apiData.error) {
        setError(apiData.error);
      } else {
        getData();
