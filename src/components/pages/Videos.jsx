...
const Testimonials = (props) => {
          ...
          <>
            <div>
              <Button
                ...
                onClick={() => {
                  largemodalShow();
                  setDeletedVideoId(props.cell.row.original._id);
  ...
  const addData = async (body) => {
    ...
    try {
      const apiData = await addVideo(body);
      if (apiData.error) {
        setError(apiData.error);
      } else {
        getData();
        handleClose3();
        toast.success(
          <p className="text-white tx-16 mb-0">Success : Video Added !</p>,
          {
            position: toast.POSITION.TOP_RIGHT,
  ...
  const updateData = async (body) => {
    ...
    try {
      if (deletedVideoIdBackend) {
        const apiData = await deleteVideo(deletedVideoIdBackend);
        if (apiData.error) {
          setError(apiData.error);
        } else {
          getData();
  ...
  return (
    <div className={styles.DataTable}>
      <PageHeader titles={`Videos`} active="Retailers" items={["Tables"]} />
                            ...
                            <div className="row">
                              <div className="col-lg-12">
                                <div className="mb-3">
                                  ...
                                  >
                                    Enter Youtube Iframe Link (Enter width="300"
                                    height="169" )
