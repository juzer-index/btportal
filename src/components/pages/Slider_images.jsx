...
const Slider_images = (props) => {
          ...
          <>
            <div>
              <Button
                onClick={() => {
                  largemodalShow();
                  setDeleteImageId(props.cell.row.original._id);
  ...
  const formsSubmitUpdate = (e) => {
    ...
    const form = e.target;
    const formData = new FormData(form);
    // formData.append("", selectedImage1Update);
    console.log("formData", formData);
    const formJson = Object.fromEntries(formData.entries());
    updateData(formJson);
    setLoading(false);
  ...
  const addData = async (body) => {
    ...
    try {
      const apiData = await addSliderImagesURL(body);
      if (apiData.error) {
        setError(apiData.error);
      } else {
        getData();
  ...
  const updateData = async (body) => {
    ...
    try {
      if (deleteImageId) {
        const apiData = await deleteSliderImage(deleteImageId);
        if (apiData.error) {
          setError(apiData.error);
        } else {
          getData();
          largemodalClose();
          toast.success(
            <p className="text-white tx-16 mb-0">Success : Image Deleted !</p>,
            {
              position: toast.POSITION.TOP_RIGHT,
              hideProgressBar: false,
              autoClose: 3000,
