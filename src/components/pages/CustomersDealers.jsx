...
import { getCustomers, getDealerCustomer } from "../../utils/api";
...
const COLUMN = [
  ...
  headersColumn("dealer", "retailer_name"),
  {
    Header: "verified",
    accessor: "verified",
    className: "text-center wd-15p border-bottom-0",
  ...
  },
  {
    ...
    Cell: (props) => (
      <Link
        to={`/admin_customers_by_id/${props.value}`}
...
const CustomersDealers = () => {
  ...
  const getData = useCallback(async () => {
    setLoading(true);
    try {
      const apiData = await getDealerCustomer(localStorage.getItem("user_id"));
      if (apiData.error) {
  ...
  return (
    <div className={styles.DataTable}>
      <PageHeader titles="My Customers" active="Customers" items={["Tables"]} />
      <ToastContainer />
...
export default CustomersDealers;
